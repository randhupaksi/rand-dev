...
Before substantial analysis or implementation, read the active machine's DevBrain runtime (when installed), this file, and the files relevant to the task, including their consumers and shared dependencies. Do not hard-code another device's user profile path.
...
`rand-dev` is the personal portfolio of **Randhu Paksi Membumi**, currently positioned in the product as a Frontend Developer with frontend, product UI, and full-stack workflow experience. The portfolio should communicate a combination of:
...
- `/contact` - direct contact channel, configured social profiles, CV link, and FAQ; this page has no submission form;
- `*` - 404 page.
...
- `src/components/sections/` contains home and feature section composition;
...
- React Hook Form and Zod are installed for validated forms if a future feature needs them; the current Contact page does not use a form
...
- The Contact page is a contact hub only; it has no form submission or API endpoint. The RandStudio project brief is a separate client-side flow in the sibling project and does not send automatically.
...
- Keep README and `.env.example` aligned with the variables actually consumed by `src/data/site.ts`.
